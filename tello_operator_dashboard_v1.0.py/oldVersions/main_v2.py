...
def to_float(d, key, default=None):
    try:
...
class TelloClient:
    ...
    def send(self, cmd: str, delay: float = 0.12):
        ...
        try:
            ...
            with reply_lock:
                last_sent_cmd = cmd
                # show intent immediately until a reply arrives
                last_cmd_reply = last_cmd_reply
        except OSError as e:
...
def main():
    ...
    # History for sparklines (last ~15 seconds at ~10Hz)
    hist_len = 150
    ...
    keepalive.enabled = True  # you can toggle with K

    # Window
    win = "Tello Operator PRO"
    ...
    try:
        while True:
            ...
            # Header
            cv2.putText(frame, "TELLO OPERATOR PRO", (18, 38),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.95, (255, 255, 255), 2)

            # Armed badge
            arm_level = "OK" if armed else "WARN"
            draw_badge(frame, "MODE", "ARMED" if armed else "DISARMED", arm_level, 18, 55)

            # SDK badge
            sdk_level = "OK" if sdk_ok else "WARN"
            draw_badge(frame, "SDK", "OK" if sdk_ok else "NOT READY", sdk_level, 18, 95)
            ...
            cv2.putText(frame, f"Sent : {sent}", (18, 345), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            cv2.putText(frame, f"Reply: {rep}", (18, 375), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            ...
            roll = st.get("roll", "?")
            vgz = st.get("vgz", "?")
            cv2.putText(frame, f"ATT  yaw {yaw}  pitch {pitch}  roll {roll}", (18, 410),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            cv2.putText(frame, f"VEL  vgx {st.get('vgx','?')}  vgy {st.get('vgy','?')}  vgz {vgz}", (18, 440),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            cv2.putText(frame, f"FPS  {fps}", (18, 470),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

            # Sparklines
            sparkline(frame, list(bat_hist), 18, 500, 240, 70, vmin=0, vmax=100, label="BAT history")
            sparkline(frame, list(h_hist),   278, 500, 240, 70, vmin=0, vmax=200, label="H history (cm)")
            sparkline(frame, list(tof_hist), 18, 585, 240, 70, vmin=0, vmax=200, label="TOF history (cm)")
            sparkline(frame, list(vxy_hist), 278, 585, 240, 70, vmin=0, vmax=60, label="Vxy (cm/s)")

            # Footer controls (operator)
            cv2.putText(frame,
                        "Keys: Q quit | A arm/disarm | T takeoff | L land (double) | C sdk | K keepalive | R record | S snapshot",
                        (18, h_img - 18),
            ...
            if key in (ord("q"), ord("Q")):
                ...
                keepalive.enabled = not keepalive.enabled
                with reply_lock:
                    last_cmd_reply = f"keepalive: {'ON' if keepalive.enabled else 'OFF'}"
                ...
                # Takeoff requires ARMED + SDK OK + battery not critical + link ok-ish
                if not armed:
                    with reply_lock:
                ...
                # LAND requires double press for safety
                if not armed:
                    with reply_lock:
                        last_cmd_reply = "blocked: DISARMED (press A)"
                    continue
